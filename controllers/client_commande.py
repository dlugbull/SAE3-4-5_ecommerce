...
def client_commande_valide():
    ...
    sql = '''SELECT declinaison_gant.id_declinaison_gant, gant.nom_gant AS nom, declinaison_gant.prix_declinaison AS prix, ligne_panier.quantite
             FROM gant
                      JOIN declinaison_gant
                           ON gant.id_gant = declinaison_gant.gant_id
                      JOIN ligne_panier
    ...
    if len(gants_panier) >= 1:
        sql = '''SELECT SUM(prix_declinaison*quantite) AS prix_total
...
def client_commande_add():
    ...
    id_client = session['id_user']
    sql = '''SELECT *, quantite*prix_declinaison AS prix
             FROM ligne_panier
    ...
    sql = '''INSERT INTO commande (date_achat, etat_id, utilisateur_id)
             VALUES (%s, 1, %s)'''
    mycursor.execute(sql, (a,id_client)) # ne fonctionne pas !!!! Manque adresses
    get_db().commit()
    ...
    for item in items_ligne_panier:
        ...
        sql = '''DELETE FROM ligne_panier
                 WHERE ligne_panier.declinaison_gant_id=%s AND ligne_panier.utilisateur_id = %s'''
        mycursor.execute(sql, (item["id_declinaison_gant"], item["utilisateur_id"]))
        get_db().commit()
        sql = '''INSERT INTO ligne_commande (commande_id, declinaison_gant_id, quantite, prix)
                 VALUES (%s, %s, %s, %s) \
              '''
